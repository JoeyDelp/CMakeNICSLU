...
void _I_NicsLU_Factorize_Cluster(SNicsLU *nicslu, SNicsLUThreadArg *tharg,
                                 unsigned int id) {
  real__t *ax;
  uint__t *ai, *ap;
  ...
  for (ii = start; ii < end; ++ii) {
    ...
    /*pivoting*/
    diagcol = p[i];
    err = _I_NicsLU_Pivot_Parallel(diagcol, &ulen[i], tmpu, tol, x, &pivcol,
                                   &pivot, lurow);
    if (FAIL(err)) {
      tharg->err = err;
      nicslu->thread_finish[id] = TRUE;
...
void _I_NicsLU_Factorize_Pipeline(SNicsLU *nicslu, SNicsLUThreadArg *tharg,
                                  unsigned int id) {
  real__t *ax;
  uint__t *ai, *ap;
  uint__t cstart, cend, oldrow;
...
int NicsLU_Factorize_MT(SNicsLU *nicslu) {
  ...
  for (ti = 0; ti < th; ++ti) {
    ...
    memset(((real__t *)(nicslu->workspace_mt1[ti])) + n, 0xff,
           sizeof(int__t) * n); /*flag*/
    memset(nicslu->workspace_mt2[ti], 0xff,
           sizeof(int__t) * (n + n)); /*updated and busy*/
    arg[ti].lnnz = 0;
    arg[ti].unnz = 0;
    arg[ti].offdiag = 0;
    arg[ti].err = NICS_OK;
  ...
  while (clv < level) {
    ...
    nicslu->thread_work =
        ((mode == 0) ? NICSLU_WORK_FACT_CLUSTER : NICSLU_WORK_FACT_PIPELINE);
    lstart = clv;

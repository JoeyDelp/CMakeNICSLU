...
int GetProcessorNumber() {
  ...
  SYSTEM_INFO si;
  PGNSI pfnGNSI = (PGNSI)GetProcAddress(GetModuleHandle("kernel32.dll"),
                                        "GetNativeSystemInfo");
  if (pfnGNSI != NULL) {
    pfnGNSI(&si);
  } else {
...
int GetProcessMemory(size_t *mem, size_t *peakmem, size_t *vmem,
                     size_t *peakvmem) {
#ifdef _WIN32
  ...
  if (NULL == mem || NULL == peakmem || NULL == vmem || NULL == peakvmem)
    return -1;
  *mem = 0;
  *peakmem = 0;
  *vmem = 0;
...
int GetBitWidth() { return (sizeof(void *) << 3); }

int GetSystemMemory(unsigned long long *phys, unsigned long long *availphys,

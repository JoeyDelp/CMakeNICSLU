...
thread_id__t _GetCurrentThread() { return GetCurrentThread(); }
...
void _Delay(unsigned int ms) { Sleep(ms); }
...
int _WaitThreadExit(thread_id__t id) { return pthread_join(id, NULL); }

thread_id__t _GetCurrentThread() { return pthread_self(); }
...
void _Delay(unsigned int ms) { usleep(ms * 1000); }

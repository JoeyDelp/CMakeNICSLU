...
char *TimerGetLocalTime(char *fmt) {
  ...
  time(&tp);
  t = localtime(&tp);
  sprintf(fmt, "%04d-%02d-%02d %02d:%02d:%02d", 1900 + t->tm_year,
...
int TimerInit(STimer *tmr) {
  ...
  tmr->start = 0;
  tmr->stop = 0;
  return (QueryPerformanceFrequency((LARGE_INTEGER *)&(tmr->freq)) == 0 ? -1
                                                                        : 0);
#else
  tmr->start.tv_sec = 0;
  tmr->start.tv_nsec = 0;
...
#ifdef _WIN32
  return (QueryPerformanceCounter((LARGE_INTEGER *)&(tmr->start)) == 0 ? -1
                                                                       : 0);
#else

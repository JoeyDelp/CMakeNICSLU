...
typedef struct tagSNicsLU {
  ...
  uint__t *row_perm; /*row_perm[i]=j-->row i in the permuted matrix is row j in
                        the original matrix*/
  uint__t *row_perm_inv; /*row_perm_inv[i]=j-->row i in the original matrix is
                            row j in the permuted matrix*/
  uint__t *col_perm;
...
int NicsLU_Initialize(INOUT__ SNicsLU *nicslu);

/*destroy the main structure and free all memories. must be called in the last*/
int NicsLU_Destroy(INOUT__ SNicsLU *nicslu);
...
int NicsLU_CreateMatrix(INOUT__ SNicsLU *nicslu, IN__ uint__t n,
                        IN__ uint__t nnz, IN__ real__t *ax, IN__ uint__t *ai,
                        IN__ uint__t *ap);
...
/*if it is called repeatedly, the previous allocated memory will be freed*/
int NicsLU_CreateScheduler(INOUT__ SNicsLU *nicslu);

/*create threads for parallel factoriztion and re-factorization*/
/*it first calls NicsLU_DestroyThreads to destroy the previously created
 * threads*/
int NicsLU_CreateThreads(
    INOUT__ SNicsLU *nicslu, IN__ unsigned int threads,
    IN__ bool__t check); /*whether to check the validity of #threads*/

/*bind threads to cores or unbind*/
int NicsLU_BindThreads(IN__ SNicsLU *nicslu, IN__ bool__t unbind);

/*destroy threads. this routine is included in NicsLU_Destroy*/
int NicsLU_DestroyThreads(INOUT__ SNicsLU *nicslu);

/*pre-processing, including ordering and static pivoting*/
int NicsLU_Analyze(INOUT__ SNicsLU *nicslu);

/*LU factorization, with partial pivoting*/
/*before called, NicsLU_Analyze must be called*/
int NicsLU_Factorize(INOUT__ SNicsLU *nicslu);

/*LU factorization, without partial pivoting*/
/*before called, NicsLU_Factorize or NicsLU_Factorize_MT must be called at least
 * once*/
int NicsLU_ReFactorize(INOUT__ SNicsLU *nicslu, IN__ real__t *ax);

/*multi-threaded version of NicsLU_Factorize*/
int NicsLU_Factorize_MT(INOUT__ SNicsLU *nicslu);

/*multi-threaded version of NicsLU_ReFactorize*/
/*before called, NicsLU_Factorize or NicsLU_Factorize_MT must be called at least
 * once*/
int NicsLU_ReFactorize_MT(INOUT__ SNicsLU *nicslu, IN__ real__t *ax);

/*solve the linear system Ax=b after LU factorization*/
int NicsLU_Solve(INOUT__ SNicsLU *nicslu,
                 INOUT__ real__t *rhs); /*for inputs, it's b, for outputs, it's
                                           overwritten by x*/

/*when there are many zeros in b, this routine is faster than NicsLU_Solve*/
int NicsLU_SolveFast(INOUT__ SNicsLU *nicslu,
                     INOUT__ real__t *rhs); /*for inputs, it's b, for outputs,
                                               it's overwritten by x*/

/*when values of A are changed but the nonzero pattern is not changed, this
 * routine resets the values*/
/*then a new LU factorization can be performed*/
/*if the nonzero pattern is also changed, call NicsLU_CreateMatrix and
 * re-preform the whole process*/
int NicsLU_ResetMatrixValues(INOUT__ SNicsLU *nicslu, IN__ real__t *ax);

/*refine the results. this routine is not always successful*/
int NicsLU_Refine(INOUT__ SNicsLU *nicslu, INOUT__ real__t *x, IN__ real__t *b,
                  IN__ real__t eps,
                  IN__ uint__t maxiter); /*if set to 0, then no constraint for
                                            iteration count*/

/*total memory access in LU factorization, including read and write, in bytes*/
/*call it after factorization*/
int NicsLU_Throughput(IN__ SNicsLU *nicslu, OUT__ real__t *thr);

/*number of floating-point operations in LU factorization*/
/*call it after factorization*/
int NicsLU_Flops(INOUT__ SNicsLU *nicslu, OUT__ real__t *flops);

/*flops of each thread*/
/*call it after factorization*/
int NicsLU_ThreadLoad(IN__ SNicsLU *nicslu, IN__ unsigned int threads,
                      OUT__ real__t **thread_flops);

/*extract A after pre-processing*/
int NicsLU_DumpA(IN__ SNicsLU *nicslu, OUT__ real__t **ax, OUT__ uint__t **ai,
                 OUT__ uint__t **ap);

/*extract LU factors after LU factorization*/
int NicsLU_DumpLU(IN__ SNicsLU *nicslu, OUT__ real__t **lx, OUT__ uint__t **li,
                  OUT__ size_t **lp, OUT__ real__t **ux, OUT__ uint__t **ui,
                  OUT__ size_t **up);

/*condition number estimation*/
/*call it after factorization*/
int NicsLU_ConditionNumber(INOUT__ SNicsLU *nicslu, OUT__ real__t *cond);

/*memory used by NICSLU, in bytes*/
/*return an approximate value*/
int NicsLU_MemoryUsage(IN__ SNicsLU *nicslu, OUT__ real__t *memuse);
...
int NicsLU_Residual(IN__ uint__t n, IN__ real__t *ax, IN__ uint__t *ai,
                    IN__ uint__t *ap, IN__ real__t *x, IN__ real__t *b,
                    OUT__ real__t *error, IN__ int norm, IN__ int mode);

/*transpose the matrix stored in CSR or CSC format*/
int NicsLU_Transpose(IN__ uint__t n, IN__ uint__t nnz, INOUT__ real__t *ax,
                     INOUT__ uint__t *ai, INOUT__ uint__t *ap);

/*sort the CSR or CSC storage*/
/*using a radix-sort like method*/
int NicsLU_Sort(IN__ uint__t n, IN__ uint__t nnz, INOUT__ real__t *ax,
                INOUT__ uint__t *ai, INOUT__ uint__t *ap);

/*merge duplicated entries in CSR/CSC*/
/*this routine also sorts the matrix*/
int NicsLU_MergeDuplicateEntries(IN__ uint__t n, INOUT__ uint__t *nnz,
                                 INOUT__ real__t **ax, INOUT__ uint__t **ai,
                                 INOUT__ uint__t **ap);
